    ...
    for i in range(10):
        ...
        if i < 5: # max steps + min steps
            ...
            for iter in range(len(data_loader_forget)):
                ...
                # compute KD loss
                loss_kd = criterionKD(outputs_forget["pred_logits"], teacher_outputs_forget["pred_logits"])
                # compute SGDA loss 
                ...
                if iter % print_freq == 0:
                    print(f"forget epoch:{epoch}, iter: {iter}, loss_kd: {loss_kd.item()}, loss_sgda: {loss_sgda.item()}, loss: {loss.item()}")
            ...
            for iter in range(len(data_loader_remain)):
                ...
                # compute KD loss
                loss_kd = criterionKD(outputs_remain["pred_logits"], teacher_outputs_remain["pred_logits"])
                # compute SGDA loss 
                loss_sgda = param_dist(student_model, swa_model, sgda_smoothing)
                ...
                # total loss
                loss_remain_total = sgda_gamma * losses_original + sgda_alpha * loss_kd + loss_sgda
                ...
                if iter % print_freq == 0:
                    print(f"remain epoch:{epoch}, iter: {iter}, loss_original:{losses_original*sgda_gamma},  
                          loss_kd: {loss_kd.item()*sgda_alpha}, loss_sgda: {loss_sgda.item()}, loss: {loss_remain_total.item()}")
                # backward
                optimizer.zero_grad()
