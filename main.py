...
def get_args_parser():
    ...
    # rehearsal rate
    parser.add_argument(
        "--rehearsal_epoch", default=50, type=int, help="rehearsal buffer epoch"
    )
    ...
    # LoRA checkpoint
    parser.add_argument(
        "--lora_checkpoint", default=None, type=str, help="checkpoint of the LoRA model"
    )
    return parser
...
def main(args):
    ...
    for phase_idx in range(total_phase_num):
        ...
        if phase_idx >= 1:
            dataset_train_balanced = build_dataset(
                image_set="train",
                args=args,
                cls_order=cls_order,
                phase_idx=phase_idx,
        ...
        if phase_idx >= 1:
            ...
            data_loader_val_new = DataLoader(
                dataset_val_new,
                args.batch_size,
        ...
        if phase_idx == 0:
            ...
            print("Testing all....")
            test_stats, coco_evaluator, all_maps = evaluate(
                model,
                criterion,
                postprocessors,
                data_loader_val,
            ...
            for epoch in range(0, args.epochs):
                ...
                if phase_idx >= 1:
                    test_stats, coco_evaluator, old_class_maps = evaluate(
                        model,
                        criterion,
                        postprocessors,
                        data_loader_val_old,
                    ...
                    test_stats, coco_evaluator, new_class_maps = evaluate(
                        model,
                        criterion,
                        postprocessors,
                        data_loader_val_new,
