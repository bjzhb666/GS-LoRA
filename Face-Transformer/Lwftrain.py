...
def L_old_kd_loss(preds, gts, temperature=2.): 
    preds = F.softmax(preds, dim=-1)
    preds = torch.pow(preds, 1./temperature)
    #l_preds = F.softmax(preds, dim=-1)
    l_preds = F.log_softmax(preds, dim=-1)


    gts = F.softmax(gts, dim=-1)
    gts = torch.pow(gts, 1./temperature)
    l_gts = F.log_softmax(gts, -1)

        
    l_preds = torch.log(l_preds)

    l_preds[l_preds != l_preds] = 0. # 去除nan值
    loss = torch.mean(torch.sum(-l_gts * l_preds, axis=1))

    return loss


def train_one_epoch_Lwf(student_model:torch.nn.Module, teacher_model:torch.nn.Module,
                        ...
                        device:torch.device,epoch:int,batch: int,
                        losses_CE:util.AverageMeter,losses_KD:util.AverageMeter,
                        losses_total:util.AverageMeter, losses_remain:util.AverageMeter,
                        ...
                        lambda_kd:float, lambda_remain:float, temperature:float):
    student_model.train()
    teacher_model.eval()
    ...
    for inputs_forget, labels_forget in iter(data_loader_cl_forget):
        ...
        outputs_forget, embeds_forget = student_model(inputs_forget.float(), labels_forget)
        loss_CE = criterion(outputs_forget, labels_forget)

        outputs_remain, embeds_remain = student_model(inputs_remain.float(), labels_remain)
        loss_remain = criterion(outputs_remain, labels_remain)

        with torch.no_grad():
            outputs_teacher, embeds_teacher = teacher_model(inputs_remain.float(), labels_remain)
            outputs_teacher = outputs_teacher.detach()
        
        loss_KD = L_old_kd_loss(outputs_remain, outputs_teacher, temperature=temperature)
        ...
        if ((batch+1)%DISP_FREQ==0) and batch!=0:
            ...
            print('Task {} Epoch {} Batch {}\t'
                    ...
                    'Training Total Loss {loss_total.val:.4f} ({loss_total.avg:.4f})\t'.format(
                        task_i, epoch+1, batch+1, loss_CE=losses_CE, loss_KD=losses_KD,
                        loss_remain=losses_remain, loss_total=losses_total))
        ...
        if ((batch+1)%VER_FREQ==0) and batch!=0:
            highest_H_mean = evaluate(student_model, 
            testloader_forget=testloader_forget, testloader_remain=testloader_remain,
