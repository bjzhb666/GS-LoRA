...
def train_one_superepoch_SCRUB(student:torch.nn.Module, teacher: torch.nn.Module, swa_model:torch.nn.Module,
                          ...
                          superepoch:int, batch:int, losses_CE:util.AverageMeter, 
                          losses_total_forget:util.AverageMeter, losses_total_remain:util.AverageMeter,
                          losses_kd_remain:util.AverageMeter, losses_kd_forget:util.AverageMeter,
                          ...
                          kd_T:float=2.0,
                          sgda_smoothing:float=0.0, sgda_gamma:float=0.99, sgda_alpha:float=0.001):
    '''
        ...
        if i<5: # max steps
            ...
            for inputs_forget, labels_forget in iter(data_loader_forget):
                ...
                if ((batch + 1) % DISP_FREQ == 0) and batch != 0:
                    ...
                    wandb.log({'epoch_loss_kd_forget-{}'.format(task_i): epoch_loss_kd_forget,
                                'epoch_loss_total_forget-{}'.format(task_i): epoch_loss_total_forget})
                    ...
                    print('Task {} Epoch {} Batch {}\t'
                            ...
                            'Forget Training total Loss {loss_total.val:.4f} ({loss_total.avg:.4f})\t'.format(
                                ...
                                loss_kd=losses_kd_forget, loss_total=losses_total_forget))
