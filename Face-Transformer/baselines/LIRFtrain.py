...
def train_one_epoch_LIRF(student_low:torch.nn.Module,deposit_low:torch.nn.Module, teacher_low:torch.nn.Module, teacher_up:torch.nn.Module,
						 data_loader_cl_forget: torch.utils.data.DataLoader, remain_loader:torch.utils.data.DataLoader,
						 criterion:torch.nn.Module, optimizer:torch.optim.Optimizer, device:torch.device, 
						 epoch: int, batch: int, # lambda_kp:float,lambda_at:float,
						 losses_CE:util.AverageMeter, losses_AT:util.AverageMeter, kd_lossesKP:util.AverageMeter, 
						 losses_pt_re:util.AverageMeter,losses_total:util.AverageMeter, losses_remain:util.AverageMeter,
						 task_i:str, testloader_forget: torch.utils.data.DataLoader, 
						 testloader_remain: torch.utils.data.DataLoader,
						 forget_acc_before: float,highest_H_mean: float, cfg:dict):
	student_low.train()
	deposit_low.train()
	criterion.train()
	...
	prefetcher_remain = data_prefetcher(remain_loader, device=device, prefetch=True) # 数据已经移动到device上
	...
	for inputs_forget, labels_forget in iter(data_loader_cl_forget):
		...
		kd_lossKP = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(student_final[:,split:]/T, dim=1),
								 F.softmax(teacher_final[:,split:]/T, dim=1)) * (cfg['LIRF_alpha']*T*T)

		# 4. Loss pt and Loss re for recovery
		loss_pt_re = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(deposit_final[:,:split]/T, dim=1),
								 F.softmax(teacher_final[:,:split]/T, dim=1)) * (cfg['LIRF_alpha']*T*T) + \
								 nn.CrossEntropyLoss()(deposit_final, labels_forget) * (1-cfg['LIRF_alpha'])
		...
		if ((batch+1)%DISP_FREQ==0) and batch!=0:
			...
			wandb.log({'epoch_loss_CE-{}'.format(task_i): epoch_loss_CE, 'epoch_loss_AT-{}'.format(task_i): epoch_loss_AT,
					   'epoch_loss_kdKP-{}'.format(task_i): epoch_loss_kdKP, 'epoch_loss_pt_re-{}'.format(task_i): epoch_loss_pt_re,
					   'epoch_loss_total-{}'.format(task_i): epoch_loss_total, 'epoch_loss_remain-{}'.format(task_i): epoch_loss_remain})
			...
			print('Task {} Epoch {} Batch {}\t'
					...
					'Training remain Loss {loss_remain.val:.4f} ({loss_remain.avg:.4f})\t'.format(
						...
						loss_CE=losses_CE, loss_AT=losses_AT, loss_total=losses_total, loss_remain=losses_remain))
		...
		if ((batch+1)%VER_FREQ==0) and batch!=0:
			highest_H_mean = evaluate_LIRF(student_low, teacher_up, testloader_forget, 
								  testloader_remain, device,batch,epoch,task_i,forget_acc_before, highest_H_mean, cfg, optimizer)
			student_low.train()
	...
	return batch, highest_H_mean, losses_CE, losses_AT, kd_lossesKP, losses_pt_re, losses_total, losses_remain



def eval_data_LIRF(student_low:torch.nn.Module, teacher_up:torch.nn.Module, 
				   testloader: torch.utils.data.DataLoader, device:torch.device,
				   mode:str, batch:int=0):
	'''
...
def evaluate_LIRF(student_low:torch.nn.Module,
	...
	# 遍历测试集
	forget_acc = eval_data_LIRF(student_low, teacher_up, testloader_forget, device, 'forget-{}'.format(task_i), batch)
	remain_acc = eval_data_LIRF(student_low, teacher_up, testloader_remain, device, 'remain-{}'.format(task_i), batch)
