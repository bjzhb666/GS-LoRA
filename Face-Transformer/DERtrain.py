...
def train_one_epoch_DER(student_model:torch.nn.Module, teacher_model:torch.nn.Module,
                        ...
                        losses_CE:util.AverageMeter,losses_DER:util.AverageMeter,
                        ...
                        lambda_der:float, plus:bool=False, lambda_der_plus:float=0.):
    student_model.train()
    teacher_model.eval()
    ...
    for inputs_forget, labels_forget in iter(data_loader_cl_forget):
        ...
        with torch.no_grad():
            outputs_teacher_remain, embeds_teacher_remain = teacher_model(inputs_remain.float(), labels_remain)
            outputs_teacher_remain = outputs_teacher_remain.detach()

        loss_der = DER_regularzaion_loss(outputs_remain, outputs_teacher_remain)

        losses_CE_next = torch.tensor(0.)
        
        if plus:
            inputs_remain_next, labels_remain_next = prefetcher_remain.next()
            outputs_remain_next, embeds_remain_next = student_model(inputs_remain_next.float(), labels_remain_next)
        ...
        if ((batch+1)%DISP_FREQ==0) and batch!=0:
            ...
            wandb.log({"epoch_loss_CE": epoch_loss_CE,
            ...
            print('Task {} Epoch {} Batch {}\t'
                    ...
                    'Training Total Loss {loss_total.val:.4f} ({loss_total.avg:.4f})\t'.format(
                        task_i, epoch+1, batch+1, loss_CE=losses_CE, loss_DER=losses_DER,
