...
def main(args):
    ...
    for task_i in range(args.num_tasks):  # start from 0
        ...
        if task_i > 0:
            ...
            print("building dataset val old...")
                ...
                if task_i == 0:
                    ...
                    if args.resume_lora is not None:
                        lora_checkpoint = torch.load(
                            args.resume_lora, map_location="cpu"
                        )
                        model_without_ddp.load_state_dict(
                            lora_checkpoint["lora"], strict=False
                        )
                else:  # task_i > 0
                    if args.one_stage:
                        last_checkpoint_path = os.path.join(
                            args.output_dir,
                            ("checkpoint_task_" + str(task_i - 1) + ".pth"),
                        )
                        checkpoint = torch.load(
                            last_checkpoint_path, map_location="cpu"
                        )
        ...
        if args.one_stage:
            ...
            for epoch in range(args.epochs):
                ...
                if args.output_dir:
                    checkpoint_paths = [
                        output_dir / ("checkpoint_task_" + str(task_i) + ".pth")
                    ]
                    checkpoint_paths_lora = [
                        output_dir / ("checkpoint_lora_task_" + str(task_i) + ".pth")
                    ]
                    ...
                    for checkpoint_path_lora in checkpoint_paths_lora:
                        utils.save_on_master(
                            {
                                "lora": OrderedDict(
                                    lora.lora_state_dict(model_without_ddp)
                                )
                            },
            ...
            utils.log_wandb(
                args=args, array=forget_maps, name="forget", task_i=task_i, epoch=0
            )
            ...
            utils.log_wandb(
                args=args, array=remain_maps, name="remain", task_i=task_i, epoch=0
            )
            if task_i > 0:
                ...
                utils.log_wandb(
                    args=args, array=old_maps, name="old", task_i=task_i, epoch=0
                )
