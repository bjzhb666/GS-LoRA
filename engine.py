...
def train_one_epoch_incremental(
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        ...
        ref_results = postprocessors["bbox"](
            ref_outputs, orig_target_sizes, topk=5, distillation=True
        )
        ref_loss_dict = criterion(outputs, ref_results, enable_aux=False)
        ref_weight_dict = criterion.ref_weight_dict
        ref_losses = sum(
            ref_loss_dict[k] * ref_weight_dict[k]
            for k in ref_loss_dict.keys()
        ...
        for img_idx in range(len(targets)):
            ...
            targets[img_idx]["boxes"] = torch.cat(
                (
                    targets[img_idx]["boxes"],
                    ref_results[img_idx]["boxes"][include_list],
                ),
                0,
            )
            targets[img_idx]["labels"] = torch.cat(
                (
                    targets[img_idx]["labels"],
                    ref_results[img_idx]["labels"][include_list],
                ),
                0,
            )
...
def train_one_epoch_forget_remain(
    model: torch.nn.Module,
    criterion: torch.nn.Module,
    ...
    metric_logger.add_meter("lr", utils.SmoothedValue(window_size=1, fmt="{value:.6f}"))
    metric_logger.add_meter(
        "forget_class_error", utils.SmoothedValue(window_size=1, fmt="{value:.2f}")
    )
    metric_logger.add_meter(
        "remain_class_error", utils.SmoothedValue(window_size=1, fmt="{value:.2f}")
    )
    metric_logger.add_meter(
        "grad_norm", utils.SmoothedValue(window_size=1, fmt="{value:.2f}")
    )
    ...
    for _ in metric_logger.log_every(
        range(len(data_loader_remain)), print_freq, header
    ):
        ...
        losses_remain = sum(
            loss_dict_remain[k] * weight_dict_remain[k]
            for k in loss_dict_remain.keys()
            if k in weight_dict_remain
        )
        ...
        losses_forget = sum(
            loss_dict_forget[k] * weight_dict_forget[k]
        ...
        loss_dict_reduced_unscaled_forget = {
            f"{k}_forget_unscaled": v for k, v in loss_dict_reduced_forget.items()
        }
        loss_dict_reduced_scaled_forget = {
            f"{k}_forget_scaled": v * weight_dict_forget[k]
        ...
        loss_dict_reduced_unscaled_remain = {
            f"{k}_remain_unscaled": v for k, v in loss_dict_reduced_remain.items()
        }
        loss_dict_reduced_scaled_remain = {
            f"{k}_remain_scaled": v * weight_dict_remain[k]
        ...
        loss_value = (
            losses_reduced_scaled_forget.item() * -1.0 * beta
            + losses_reduced_scaled_remain.item()
        )
...
def train_one_epoch_forget_cls(
    ...
    for _ in metric_logger.log_every(
        ...
        for k in loss_dict_forget.keys():
            if k in weight_dict_forget:
                if weight_dict_forget[k] < 0:
                    losses_forget += torch.functional.F.relu(
                        BND + loss_dict_forget[k] * weight_dict_forget[k]
                    )
                else:
        ...
        loss_value = (
            losses_reduced_scaled_forget.item() * beta
            + losses_reduced_scaled_remain.item()
            + alpha * structure_loss.item()
        )
...
def group_sparse_loss(model: torch.nn.Module, type="layer"):
    ...
    for i in range(12):
        ...
        if i < 6:
            group_item.append(
                "transformer.encoder.layers." + str(i) + ".linear1.lora_A"
            )
            group_item.append(
                "transformer.encoder.layers." + str(i) + ".linear1.lora_B"
            )
            group_item.append(
                "transformer.encoder.layers." + str(i) + ".linear2.lora_A"
            )
            group_item.append(
                "transformer.encoder.layers." + str(i) + ".linear2.lora_B"
            )
        else:
            group_item.append(
                "transformer.decoder.layers." + str(i - 6) + ".linear1.lora_A"
            )
            group_item.append(
                "transformer.decoder.layers." + str(i - 6) + ".linear1.lora_B"
            )
            group_item.append(
                "transformer.decoder.layers." + str(i - 6) + ".linear2.lora_A"
            )
            group_item.append(
                "transformer.decoder.layers." + str(i - 6) + ".linear2.lora_B"
            )
