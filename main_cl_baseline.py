...
def get_args_parser():
    ...
    # baseline CL method
    parser.add_argument('--l2',default=False,action='store_true',help='whether to use l2 regularization')
...
def main(args):
    ...
    if args.rank == 0:
        ...
        wandb.init(project="forget learning", dir=args.output_dir, \
                    group="cl-baseline", mode="offline" if args.wandb_offline else "online")
    ...
    for task_i in range(args.num_tasks): # start from 0
        ...
        if args.replay:
            ...
            dataset_total = torch.utils.data.ConcatDataset([dataset_remain, dataset_cl_forget])
        ...
        batch_sampler_remain = torch.utils.data.BatchSampler(sampler_remain,
                                                            args.batch_size,
                                                            drop_last=True)
        batch_sampler_cl_forget = torch.utils.data.BatchSampler(sampler_cl_forget,
                                                            args.batch_size,
                                                            drop_last=True)
        if args.replay:
            batch_sampler_total = torch.utils.data.BatchSampler(sampler_total,
                                                            args.batch_size,
                                                            drop_last=True)
        ...
        data_loader_val_remain = DataLoader(dataset_val_remain, batch_size=args.batch_size, \
                                            ...
                                            collate_fn=utils.collate_fn, num_workers=args.num_workers, pin_memory=True)
        data_loader_cl_forget = DataLoader(dataset_cl_forget,
            ...
            for l2_epoch in range(args.epochs):
                ...
                if args.replay:
                    train_stats = train_one_epoch_l2(model=model,criterion=criterion,
                                                        ...
                                                        device=device,optimizer=optimizer,epoch=l2_epoch,
                                                        clip_max_norm=args.clip_max_norm,
                                                        l2_lambda=args.l2_lambda,regularization_terms=regularization_terms)
                else: # no replay                                   
                    train_stats = train_one_epoch_l2(model=model,criterion=criterion,
                ...
                test_stats_forget, coco_evaluator_forget, forget_maps = evaluate(
                    model, criterion, postprocessors, data_loader_val_forget,
                    base_ds_forget, device, args.output_dir)
                utils.log_wandb(args=args,array=forget_maps,name='forget',task_i=task_i, epoch=l2_epoch+1)
        ...
        if args.ewc:
            ...
            for ewc_epoch in range(args.epochs):
                ...
                test_stats_forget, coco_evaluator_forget, forget_maps = evaluate(
                    model, criterion, postprocessors, data_loader_val_forget,
                    base_ds_forget, device, args.output_dir)
                utils.log_wandb(args=args,array=forget_maps,name='forget',task_i=task_i, epoch=ewc_epoch+1)
