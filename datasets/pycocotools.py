...
class COCO:
    ...
    def createIndexByCls(
        ...
        if balanced_ft:
            ...
            non_selected_cls_this_phase = [
                cls for cls in cls_order if cls not in selected_cls_this_phase
            ]
                ...
                if tfs_or_tfh == "tfh":
                    ...
                    if is_rehearsal:
                        if not args.few_shot:  # not few-shot
                            ...
                            selected_img_id_array = random.sample(
                                this_phase_all_img_id_list, num_selected_img_this_phase
                            )
                        else:
                            print("\033[92musing few-shot setting\033[0m")
                            num_selected_img_this_phase = args.few_shot_num * len(
                                selected_cls_this_phase
                            )
                            try:
                                selected_img_id_array = random.sample(
                                    this_phase_all_img_id_list,
                                    num_selected_img_this_phase,
                                )
                            except ValueError as e:
                                print(
                                    "\033[91mShot number is larger than the number of images in the dataset.\033[0m"
                                )
                                selected_img_id_array = random.choices(
                                    this_phase_all_img_id_list,
                                    k=num_selected_img_this_phase,
                                )
                    else:  # forget_rate of data is used for training
                        if not args.few_shot:
                            num_selected_img_this_phase = int(
                                num_img_this_phase * args.forget_rate
                            )
                            selected_img_id_array = random.sample(
                                this_phase_all_img_id_list, num_selected_img_this_phase
                            )
                            # print("selected_img_id_array: ", selected_img_id_array)
                        else:
                            num_selected_img_this_phase = args.few_shot_num * len(
                                selected_cls_this_phase
                            )
