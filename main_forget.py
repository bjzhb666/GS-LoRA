...
from engine import (
    evaluate,
    train_one_epoch,
    train_one_epoch_incremental,
    train_one_epoch_forget_remain,
    train_one_epoch_forget_cls,
)
...
def main(args):
    ...
    if args.sgd:
        ...
        optimizer_remain = torch.optim.SGD(
            param_dicts, lr=args.lr, momentum=0.9, weight_decay=args.weight_decay
        )
    else:
        ...
        optimizer_remain = torch.optim.AdamW(
            param_dicts, lr=args.lr, weight_decay=args.weight_decay
        )
    ...
    if args.resume:
        if args.resume.startswith("https"):
            ...
            if args.resume_lora is not None:
                lora_checkpoint = torch.load(args.resume_lora, map_location="cpu")
                model_without_ddp.load_state_dict(lora_checkpoint["lora"], strict=False)
    ...
    if args.one_stage:
        ...
        for epoch in range(args.epochs):
            ...
            utils.log_wandb(
                args=args, array=forget_maps, name="forget", epoch=epoch + 1
            )
            ...
            utils.log_wandb(
                args=args, array=remain_maps, name="remain", epoch=epoch + 1
            )
    ...
    if args.rehearsal_training:
        ...
        for remain_epoch in range(args.finetune_rehearsal_epoch):
            ...
            utils.log_wandb(
                args=args, array=forget_maps, name="forget", epoch=remain_epoch + 1
            )
            ...
            utils.log_wandb(
                args=args, array=remain_maps, name="remain", epoch=remain_epoch + 1
            )
    ...
    if args.finetune_gradient_ascent:
        ...
        for forget_epoch in range(args.finetune_forget_epoch):
            ...
            utils.log_wandb(
                args=args, array=forget_maps, name="forget", epoch=forget_epoch + 1
            )
            ...
            utils.log_wandb(
                args=args, array=remain_maps, name="remain", epoch=forget_epoch + 1
            )
    ...
    else:  # Two stage training, train the forget set first, then train the remain set
...
if __name__ == "__main__":
    ...
    # finish the wandb run and rename the run
    if args.rank == 0:
        if args.one_stage:
            wandb.run.name = (
                "forget"
