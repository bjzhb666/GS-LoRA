...
def main(args):
    ...
    #     phase_idx=0, incremental=True, incremental_val=False, val_each_phase=False)
    # dataset_remain = build_dataset(image_set='train', args=args, cls_order=cls_order, \
    #     phase_idx=1, incremental=True, incremental_val=False, val_each_phase=False, is_rehearsal=True)
    ...
    if args.one_stage:
        ...
        for epoch in range(args.epochs):
            ...
            # train_stats = train_one_epoch_forget_remain(model,
            #                                             criterion,
    ...
    for phase_idx in range(total_phase_num):
        ...
        if args.distributed:
            if args.cache_mode:
                ...
                sampler_val = samplers.NodeDistributedSampler(dataset_val,
                                                              shuffle=False)
                if phase_idx >= 1:
                    sampler_train_balanced = samplers.NodeDistributedSampler(
                        dataset_train_balanced)
                    sampler_val_old = samplers.NodeDistributedSampler(
                        dataset_val_old, shuffle=False)
                    sampler_val_new = samplers.NodeDistributedSampler(
                        dataset_val_new, shuffle=False)
            else:
                sampler_train = samplers.DistributedSampler(dataset_train)
                sampler_val = samplers.DistributedSampler(dataset_val,
                                                          shuffle=False)
                if phase_idx >= 1:
                    sampler_train_balanced = samplers.DistributedSampler(
                        dataset_train_balanced)
                    sampler_val_old = samplers.DistributedSampler(
                        dataset_val_old, shuffle=False)
                    sampler_val_new = samplers.DistributedSampler(
                        dataset_val_new, shuffle=False)
        else:
            sampler_train = torch.utils.data.RandomSampler(dataset_train)
            sampler_val = torch.utils.data.SequentialSampler(dataset_val)
            if phase_idx >= 1:
                sampler_train_balanced = torch.utils.data.RandomSampler(
                    dataset_train_balanced)
                sampler_val_old = torch.utils.data.SequentialSampler(
                    dataset_val_old)
                sampler_val_new = torch.utils.data.SequentialSampler(
                    dataset_val_new)
        ...
        data_loader_val = DataLoader(dataset_val,
                                     ...
                                     pin_memory=True)
        if phase_idx >= 1:
            data_loader_train_balanced = DataLoader(
                dataset_train_balanced,
                batch_sampler=batch_sampler_train_balanced,
                collate_fn=utils.collate_fn,
                num_workers=args.num_workers,
                pin_memory=True)
            data_loader_val_old = DataLoader(dataset_val_old,
                                             args.batch_size,
                                             ...
                                             num_workers=args.num_workers,
                                             pin_memory=True)
            data_loader_val_new = DataLoader(dataset_val_new,
                                             args.batch_size,
        ...
        if phase_idx >= 1:
            if args.sgd:
                optimizer_balanced = torch.optim.SGD(
                    param_dicts,
                    lr=args.lr,
                    momentum=0.9,
                    weight_decay=args.weight_decay)
            ...
            lr_scheduler_balanced = torch.optim.lr_scheduler.StepLR(
                optimizer_balanced, args.lr_drop_balanced)
        ...
        if phase_idx == 0:
            ...
            if os.path.exists(ckpt_path):
                ...
                missing_keys, unexpected_keys = model_without_ddp.load_state_dict(
                    checkpoint['model'], strict=False)
                unexpected_keys = [
                    k for k in unexpected_keys if not (
        ...
        else:  # phase_idx >= 1, incremental training
            if phase_idx >= 1:
            ...
            for epoch in range(0, args.epochs):
                ...
                if phase_idx >= 1:
                    train_stats = train_one_epoch_incremental(
                        model, old_model, args.ref_loss_overall_coef,
                        criterion, postprocessors, data_loader_train,
                ...
                if phase_idx >= 1:
                    test_stats, coco_evaluator = evaluate(
                        ...
                        base_ds_old, device, args.output_dir)  # class 0-40
                    print("Testing results for old.")
                    test_stats, coco_evaluator = evaluate(
                        model, criterion, postprocessors, data_loader_val_new,
                        base_ds_new, device, args.output_dir)  # class 41-80
                    print("Testing results for new.")

            # rehearsal training, only for phase_idx >= 1, 20 epochs
            if args.balanced_ft and phase_idx >= 1:
                for epoch in range(0, 20):
                    ...
                    train_stats = train_one_epoch(model, criterion,
                                                  data_loader_train_balanced,
                                                  optimizer_balanced, device,
                    ...
                    if phase_idx >= 1:
                        test_stats, coco_evaluator = evaluate(
                            ...
                            data_loader_val_old, base_ds_old, device,
                            args.output_dir)
                        print("Balanced FT - Testing results for old.")
                        test_stats, coco_evaluator = evaluate(
                            model, criterion, postprocessors,
                            data_loader_val_new, base_ds_new, device,
                            args.output_dir)
